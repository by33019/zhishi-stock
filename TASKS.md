...
- [x] **M3-09** P1 AI 配额与用量统计 — 已完成，见下方详情
  ...
  > 两条口径各自只有一处产出，互不替代。
- [x] **M3-10** P0 前端 ai 工作台 + history 接真实 API/SSE — 已完成，见下方详情
  > `/history` 接真实接口（HIS-01/02/03/04/05，含改名/收藏/两步删除）；
  > `/ai` 工作台接真实接口：AI-01/02/03/04 + HIS-06/07 + USER-07，
  > **运行期走 SSE（AI-05）**——自建 fetch 流通道（`apiStream` + `sseParser`），
  > 支持 `Last-Event-ID` 断线续传、401 刷新一次、重连上限 3 次后退回 3 秒轮询；
  > 取消/重试/追问（AI-06/07/08）全部接入；板块与多标的对比场景的标的检索
...
**交付范围**：契约 §13.3 的报告反馈两个端点。`ai_feedback` 表自 V6 建立起"结构就绪、无代码引用"，
本轮接上应用层；同时把 HIS-06 里"如实为 `null`"的 `feedback` 字段变成真实数据。
**AI 域的报告生命周期自此完整**：生成 → 落库 → 读回 → 用户评价 → 撤销评价。

| 项 | 内容 |
| --- | --- |
| HIS-08 | `PUT /api/v1/ai/reports/{reportId}/feedback`（`USER`）——创建或替换本人唯一反馈 |
| HIS-09 | `DELETE /api/v1/ai/reports/{reportId}/feedback`（`USER`）——返回 `deleted` |
| 领域层 | `AiFeedback`（含 `MAX_DETAIL_LENGTH`）、`AiFeedbackType`、`AiFeedbackReasonCode`、`AiFeedbackStore` |
| 持久化 | `AiFeedbackRow` / `AiFeedbackMapper`（`ON DUPLICATE KEY UPDATE`）/ `MyBatisAiFeedbackStore` |
| 用例层 | `AiFeedbackService`、`AiFeedbackView`、`InvalidAiFeedbackException`（400 `INVALID_REQUEST`） |
| 装配 | `BackendConfiguration` 两个新 Bean；**`AiWorkerConfiguration` 刻意不加**——worker 不碰反馈 |
| 测试 | `AiFeedbackServiceTest` 7 项、`AiReportControllerContractTest` 由 6 增至 10 项 |

### 关键取舍

1. **写路径只有 `upsert` 一个动词，且写完必须回读**。撞 `uk_ai_feedback_report_user` 时数据库
   **保留原 id 与 `created_at`**，本次的自增 id 被丢弃——把入参当结果返回，调用方拿到的
   `feedbackId` 会指向不存在的行，而前端接下来正要用它去删。单测专门断言"来自重读"。
2. **`ON DUPLICATE KEY UPDATE` 而非"先查再插改"**：后者并发下两个请求都判为"不存在"，
   其中一个撞唯一索引报错，而"同一个人连点两次评价"完全正常。刻意不更新 `created_at`。
3. **`detail` 上限只定义一次**（`AiFeedback.MAX_DETAIL_LENGTH`，与 `ck_ai_feedback_detail` 的 300 对齐）。
   三处各写一个 300 必然漂移，漂移的表现是"应用层放过、数据库拒绝"。
4. **空白裁剪、空串按"未提供"**：不让"没填原因"与"填了空原因"在数据里看起来不同。
5. **重复删除返回 `deleted=false` 而不是 404**（契约该接口的响应字段就是 `deleted`）。
6. **归属校验复用 `AiReportQueryService.requireOwned`**：写反馈与读报告必须同一条判据，
   否则分叉成"读不到的报告却能打分"——不会报错的越权。
7. **业务码用通用 `INVALID_REQUEST`**，不自造 `AI_FEEDBACK_INVALID`：契约未为反馈定义专属码。

### 验证结果

真实端到端 9 项检查全通过（Docker 全栈 + HIS-06 那份真实报告）：
初始 `null` → PUT 200（`feedbackId` 为字符串）→ HIS-06 一次带回 → **再次提交 `feedbackId` 不变**
...
**交付范围**：`/ai` 工作台从「轮询 + 仅单标的」补齐为契约 AI-01~08 全量 + USER-07；
`/history` 此前已接（HIS-01~05）。本轮收口后，**M3 仅剩 M3-11（后台 admin 最小集）**。

| 项 | 内容 |
| --- | --- |
| SSE 解析 | `services/sseParser.ts`：WHATWG event-stream 最小子集（event / id / data / 注释行），字符串进字符串出的纯函数——解析器的错误只表现为"安静地收不到事件"，必须能在单测里钉住分帧边界 |
| SSE 通道 | `apiClient.apiStream`：不用 `EventSource`（它带不上内存里的 `Authorization` 头）；`Last-Event-ID` 断线续传、401 刷新**一次**（刷新后再 401 必须走失败路径，否则是没有退避的死循环）、重连上限 3 次后回调 `onError`、**刻意不设请求超时**（流本来就长时间静默） |
| 轮询兜底 | 流不可用（网关不支持 / 重连用尽）时退回 3 秒轮询 AI-04——闭环不能因为一条流挂掉就整体不可用；页面如实提示"实时流不可用"而不是静默降级 |
| 事件去重 | 页面按 `sequence` 推进游标，重连补发的重复 chunk 不会把同一段临时文本拼两遍 |
| AI-06/07/08 | 取消（`effectiveImmediately=false` 如实说"未生效"，不说"已取消"）、重试（仅 FAILED / TIMED_OUT，创建**新任务**并接上新的事件流）、追问（同一会话 `follow-up-tasks`） |
| 标的检索 | 目标选择形态由 AI-01 的场景定义推导（不在前端硬编码场景清单）：MARKET 固定 `CN`；SECURITY 走 STK-01（单选或 2~3 只多选）；SECTOR 走 SEC-02 榜单取候选后本地过滤（一次返回全部板块行情，不需要后端新增检索接口） |
| nginx | `/api/v1/ai/` 专用 location：`proxy_buffering off`（否则事件被攒进缓冲区，"逐字出现"变成"一次性倾泻"）+ `proxy_read_timeout 300s` + HTTP/1.1 长连接 |
| 浏览器验收 | `e2e/ai-workspace.real.mjs`：31 项全通过；覆盖 /ai 真实提交（AI-01/02/03/05/06/07/08 + HIS-06/07 + USER-07）与 /history 三个写操作（HIS-03/04），并**直接查库**核对（ai_task 落库、报告截止时间为真实行情批次、页面免责声明与库中一致、改名/收藏/软删落库） |

### 关键设计取舍

1. **SSE 是主路，轮询是兜底，两者共用同一套状态机。** AI-04 本来就能表达
   "排队中 / 运行中 / 已完成"，所以降级不是残次品，只是少了中间态的临时文本。
2. **未知事件名与解析失败的载荷被跳过而不是抛错。** 契约升级多出一个新事件时，
   旧前端应当照常工作；半截 JSON 重连补发一次就好。
3. **目标数量不够就不给提交**（`targetsValid` 按场景定义的 min/max 判定），
   不把这个判断推给服务端 400——用户不该白跑一次才知道"对比至少要 2 只"。
4. **`done` 之后以 AI-04 的任务摘要为准再对齐一次。** status 事件可能只是中间态，
   终态的权威来源是任务查询接口。
5. **查库用异步 `execFile` 不用 `execFileSync`**：本机环境 Node 的 spawnSync 一律
   EBUSY（连 `cmd.exe` 都起不来），异步路径正常。e2e 脚本里已注明。

### 验证结果

- 前端 `npm run typecheck` 0 错误；`vitest` **24 文件 / 211 项全绿**（+18：
  sseParser 7、apiClient SSE 通道 5、AI 工作台新增/改写 6）。
- 浏览器级验收（Docker 全栈 7 容器 + Playwright，经 nginx `:8088`）：
  登录 → AI-01/USER-07 → STK-01 检索 → AI-02 预览 → AI-03 **202** →
  SSE 流建立（`/ai/tasks/{id}/stream` 200）→ **临时文本逐段增长**（最终 695 字符）→
  报告六章节 + 数据截止 + 引用 → 查库（`ai_task` COMPLETED、
  `market_data_cutoff_at = 2026-09-23 15:00:00`、`ai_message` 2 条）→
  AI-08 追问同会话落第二个任务 → /history 改名（PATCH 200，库中标题更新）→
  收藏（`is_favorite=1`）→ 两步删除（`deleted_at` 非空、列表消失）。
- **Chrome 把主动 `close()` 的 SSE 分块响应记为 `ERR_INCOMPLETE_CHUNKED_ENCODING`
  控制台噪声**——这是断流的预期表现，验收脚本按白名单过滤并注明原因；
  流是否健康由"文本逐段增长 + 报告就位 + 库中 COMPLETED"三项共同证明。
