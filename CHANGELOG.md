...
- `services/historyApi.ts` 新增 `updateSession` / `deleteSession`（带 `If-Match`）；
  `types/domain.ts` 新增 `AiSessionUpdated` / `AiSessionDeletion`。
- 详情区新增操作组：收藏 / 取消收藏、重命名（内联输入）、删除（**两步确认**）。
- `HistoryPage.test.ts` 由 7 增至 **11** 项；`business.css` 新增操作组、重命名表单、
  删除确认区样式。

### 关键取舍

1. **`If-Match` 用页面当前渲染的那一版提交**，不在 service 里自己取版本。
   自己取就成了"用最新版写"，乐观锁完全失效。单测断言传给服务端的是
   **详情里那一版**（`version: 7`），不是刷新后的。
2. **409 单独处理**。它不是"操作非法"，而是"这份会话在你操作期间被别人改过"。
   此时正确响应是**重新拉取并请用户再试**，而不是把"版本冲突"原样抛给他——
   用户改一次标题看到这四个字，无从知道下一步做什么。
3. **删除两步确认**：一次点击就删掉一条研究会话太容易误触。
4. **写成功后重新拉取而不是本地改**：服务端会改写 `version`，本地那份立刻过期；
   而且删除会改变列表构成。

### 测试查出的一个真实缺陷（已修）

`actionNotice` / `actionError` 最初渲染在详情区**内部**，而删除成功后 `selectedId` 被清空、
详情区随之消失——**用户点完删除只看到列表少了一行，看不到"已删除，10/22 之后彻底清理"**。
提示块已移到详情区之外。这条是单元测试发现的：断言"删除后文案可见"失败，
而症状在界面上表现为"删除没有反馈"。

### 验证

- `npm run typecheck` 0 错误；`npm run build` 成功（`HistoryPage` 由 7.81 kB 增至 **10.45 kB**）；
  前端 **21 文件 / 161 项**测试全绿（新增 4 项：收藏用当前版本提交、重命名与空标题拦截、
  409 提示并刷新、两步删除与清理时间提示）。
- 浏览器验收脚本 `e2e/history.real.mjs` 重跑 **13 项全通过**（未回归）。
  **注意**：该脚本**尚未覆盖**三个写操作——本轮尝试扩展时，
  用 `python -c "…"` 写入含反引号的 JS 模板串被 shell 当命令替换执行、脚本损坏，
  已回滚到已提交版本。写操作目前只有单测覆盖。

### 未覆盖 / 待办

- **浏览器级的写操作验收缺失**（见上）。补的时候注意：列表按最后活动时间倒序，
  **第一行是最新的真实报告**，别拿它做写操作或删除，会毁掉其它验证的参照数据。
- `formatDate` 只输出 `MM/DD`，用在"30 天后清理"这类跨月/跨年的未来日期上
  缺少年份。属 `utils/format` 的既有口径，本轮未动。
