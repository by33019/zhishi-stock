...
> 最后更新：2026-09-20（M3-02 完成：自选项 CRUD + 排序 + 行情概览，关闭已知问题 #15）
...
| 前端类型检查 + 测试 + 构建 | ✅ 通过 | **19 文件 / 101 测试**（默认时区与 `TZ=UTC` 下均全绿）；`npm run typecheck` 0 错误；`vite build` 成功 |
...
| M3 | 用户态闭环与 AI 研究编排 | 🟡 **2/12 完成**（M3-01 自选分组 CRUD、M3-02 自选项 CRUD + 排序 + 行情概览；自选中心后端 WAT-01~WAT-12 全部可用） |
...
**下一步：M3 已开工（2/12）**，按契约依赖推进：

1. ✅ M3-01 自选分组 CRUD → ✅ M3-02 自选项 CRUD + 排序 + 行情概览（自选中心后端 12 个接口全部可用）
2. **M3-03 前端 `/watchlist` 接真实 API** —— 依赖 M3-02，已具备全部数据源
3. **STK-05 批量行情接口**（已知问题 #10）—— M3-03 自选页首屏行情与 M2-09 遗留的"搜索建议无涨跌幅"
   都依赖它；底层 `SimulatedQuoteSnapshotProvider` 已同时提供单只与整批，实现成本很低，建议紧随 M3-03
