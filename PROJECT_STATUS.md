...
| 后端全量测试 | ✅ **426 测试通过** | common 1 / system 45 / market 156 / integration 106 / backend 116 / job 2，0 失败 0 错误。本机 Docker 已启动，`InfrastructureIntegrationTest`（Testcontainers 真实 MySQL 8.4 + Redis）**本轮实测通过**，不再是"CI 覆盖" |
...
| 后端 | 🟡 4/8 域 | 认证闭环 ✅、市场总览（MKT-01~04）🟡、证券主数据与个股行情（STK-01/02/04/07）🟡、榜单（QTE-01）🟡、板块（SEC-01/02/03/04/06）🟡、**自选分组（WAT-01~WAT-05）🟡**；其余未开工 |
| 测试 | ✅ 527 个 | 后端 426 + 前端 101；无覆盖率门槛 |
...
**下一步：进入 M3（用户态闭环与 AI 研究编排，12 个任务）**。建议起点 `M3-01`（自选分组 CRUD，V5 表已就绪）。
另有一项建议补入 M3 的小任务：**STK-05 批量行情**（已知问题 #10）——契约 §8 明列该接口，
底层 `SimulatedQuoteSnapshotProvider` 已同时提供单只与整批，实现成本很低，
且 `M3-03` 自选页首屏行情与 M2-09 遗留的"搜索建议无涨跌幅"都依赖它。
