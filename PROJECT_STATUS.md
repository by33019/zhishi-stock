...
| 后端全量测试 | ✅ **353 测试通过** | 全量 354 项；其中 1 项 `InfrastructureIntegrationTest`（Testcontainers）在本机因未启动 Docker 守护进程报 `Could not find a valid Docker environment`，CI 可跑 |
...
| 测试 | ✅ 454 个 | 后端 354 + 前端 101（后端含 1 项需 Docker 的集成测试）；无覆盖率门槛 |
